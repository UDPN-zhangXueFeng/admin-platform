 ...
 *
 * The token cookie is set by `setAccessToken()` in the browser (dual
 * localStorage + cookie strategy). Middleware only reads the cookie —
 * it cannot access localStorage.
 */

const intlMiddleware = createMiddleware(routing);

/** Paths that don't require authentication */
const PUBLIC_PATHS = ['/login', '/api', '/aps', '/_next'];
