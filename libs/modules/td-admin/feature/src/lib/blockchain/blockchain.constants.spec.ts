...
describe('i18n key 前缀常量（动态拼接）', () => {
  ...
  it('builds the node status color key common_task_status_color_${status}', () => {
    expect(`${NODE_STATUS_COLOR_KEY_PREFIX}1`).toBe(
      'common_task_status_color_1',
    );
    expect(`${NODE_STATUS_COLOR_KEY_PREFIX}2`).toBe(
      'common_task_status_color_2',
    );
  });

  it('builds the node status label key node_status_${status}', () => {
    expect(`${NODE_STATUS_LABEL_KEY_PREFIX}1`).toBe('node_status_1');
    expect(`${NODE_STATUS_LABEL_KEY_PREFIX}2`).toBe('node_status_2');
  });

  it('builds the tokenType label key token_type_${n}', () => {
    expect(`${TOKEN_TYPE_LABEL_KEY_PREFIX}2`).toBe('token_type_2');
  });

  it('builds the deployment type label key type_${n}', () => {
    expect(`${DEPLOYMENT_TYPE_LABEL_KEY_PREFIX}1`).toBe('type_1');
    expect(`${DEPLOYMENT_TYPE_LABEL_KEY_PREFIX}5`).toBe('type_5');
  });
...
describe('通用常量', () => {
  it('ALL_VALUE is the non-empty sentinel "all" (Select 全部 = 不筛选)', () => {
    // FormSelect 不支持空串 value，源码约定 ALL_VALUE = 'all'，筛选时 !== ALL_VALUE 判断。
